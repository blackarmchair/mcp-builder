...
const MCPIcon = ({ icon, size, invert }) => {
	...
	const icons = {
		...
		failure: {
			keys: ['{failure}', 'failure', '{fail}', 'fail'],
			icon: FAILURE,
		},
		...
		size: { keys: ['{size}', 'size', '{sz}', 'sz'], icon: SIZE },
		speed: { keys: ['{speed}', 'speed', '{spd}', 'spd'], icon: SPEED },
		stamina: {
			keys: ['{stamina}', 'stamina', '{stam}', 'stam'],
			icon: STAMINA,
		},
		strength: {
			keys: ['{strength}', 'strength', '{str}', 'str'],
			icon: STRENGTH,
		},
		threat: { keys: ['{threat}', 'threat', '{thr}', 'thr'], icon: THREAT },
